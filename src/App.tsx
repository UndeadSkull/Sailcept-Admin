      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
        <Card title="January 2025">
          <div className="grid grid-cols-7 gap-2">
            {days.map((day) => {
              const style = day % 5 === 0 ? 'bg-amber-100 text-amber-700' : day % 3 === 0 ? 'bg-slate-100 text-slate-500' : 'bg-emerald-100 text-emerald-700'
              return (
                <button
                  key={day}
                  type="button"
                  className={`rounded-lg px-2 py-3 text-xs font-medium ${style}`}
                >
                  {day}
                </button>
              )
...
function AppLayout() {
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-100 via-cyan-50 to-blue-100 text-slate-700">
