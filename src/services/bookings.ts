...
import { Booking, BookingRequest, DayBooking, DietEntry } from "../data/bookings";
export type { Booking, BookingRequest, DayBooking, DietEntry };
...
export let reviews = [...initialReviews];
...
export async function fetchReviews(boatId: number): Promise<ApiResponse<typeof reviews>> {
  await delay(400);
  const boatName = BOAT_NAME_MAP[boatId];
  if (boatId === 0 || !boatName) {
    return { data: reviews, error: null };
  }
  const filtered = reviews.filter((r) => r.boat === boatName);
  return { data: filtered, error: null };
}
